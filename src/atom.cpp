...
void Atom::peratom_create()
{
  ...
  add_peratom("Ne",&Ne,DOUBLE,0);
  add_peratom("dE_dN",&dE_dN,DOUBLE,0); 
...
void Atom::data_electronnumber(const char *file, int line, const char *str)  //?
{
  Ne = new double[1];
  double Ne_one;
  auto location = "Electron Number section of data file";
  ...
  if (nwords != 1) error->all(file, line, "Invalid format in {}: {}", location, str);
  
  Ne_one = utils::numeric(file, line, values[0], false, lmp);

  //if (Ne_one <= 0.0)
  //error->all(file, line, "Invalid electron number {} in {}: {}", Ne_one, location, utils::trim(str));
...
void Atom::data_dE_dN(const char *file, int line, const char *str)  //?
{
  dE_dN = new double[1];
  double dE_dN_one;
  auto location = "dE/dN section of data file";
  auto values = Tokenizer(str).as_vector();
  int nwords = values.size();
  for (std::size_t i = 0; i < values.size(); ++i) {
    if (utils::strmatch(values[i], "^#")) {
  ...
  dE_dN_one = utils::numeric(file, line, values[0], false, lmp);
...
.. list-table::
   ...
   * - dE_dN
     - double
     - 1
     - Similar to the force dE_dN on the number of electrons
   * - Ne
     - double
     - 1
     - number of electrons of the particles
...
void *Atom::extract(const char *name)
{
  ...
  if (strcmp(name,"dE_dN") == 0) return (void *) dE_dN;  //?
  if (strcmp(name,"Ne") == 0) return (void *) Ne;  //?
...
int Atom::extract_datatype(const char *name)
{
  ...
  if (strcmp(name,"dE_dN") == 0) return LAMMPS_DOUBLE; //?
  if (strcmp(name,"Ne") == 0) return LAMMPS_DOUBLE; //?
