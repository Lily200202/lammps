...
int PairDeepMDNew::get_node_rank() {
...
std::string PairDeepMDNew::get_file_content(const std::string &model) {
  int myrank = 0, root = 0;
...
std::vector<std::string> PairDeepMDNew::get_file_content(
...
void PairDeepMDNew::make_fparam_from_compute(vector<double> &fparam) {
...
void PairDeepMDNew::make_aparam_from_compute(vector<double> &aparam) {
...
#ifdef USE_TTM
void PairDeepMDNew::make_ttm_fparam(vector<double> &fparam) {
  assert(do_ttm);
  // get ttm_fix
...
#ifdef USE_TTM
void PairDeepMDNew::make_ttm_aparam(vector<double> &daparam) {
  assert(do_ttm);
  // get ttm_fix
  const FixTTMDP *ttm_fix = NULL;
  for (int ii = 0; ii < modify->nfix; ii++) {
...
void PairDeepMDNew::cum_sum(std::map<int, int> &sum, std::map<int, int> &vec) {
  sum[0] = 0;
...
PairDeepMDNew::PairDeepMDNew(LAMMPS *lmp)
    : Pair(lmp)

{
  ...
  ener_unit_cvt_factor = force->boltz / 8.617343e-5;  //?不同单位转换因子
...
void PairDeepMDNew::print_summary(const string pre) const {
  if (comm->me == 0) {
...
void PairDeepMDNew::compute(int eflag, int vflag) {
  if (numb_models == 0) {
    return;
  }
  if (eflag || vflag) {
  ...
  if (do_ghost) {
    ...
    if (single_model || multi_models_no_mod_devi) {
      ...
      if (!(eflag_atom || cvflag_atom)) {
        if (!atom->sp_flag) {
          try {
            ...
            deep_pot.compute(dener, dforce, dvirial, ddE_dN, dcoord, dtype, dbox,
          ...
          try {
            deep_pot.compute(dener, dforce, dvirial, ddE_dN, extend_dcoord,
      ...
      // do atomic energy and virial
        ...
        if (!atom->sp_flag) {
          try {
            deep_pot.compute(dener, dforce, dvirial, ddE_dN, deatom, dvatom, dcoord,
    ...
    if (numb_models == 1) {
      try {
        deep_pot.compute(dener, dforce, dvirial, ddE_dN, dcoord, dtype, dbox);  //?1
      } catch (deepmd_compat::deepmd_exception &e) {
        error->one(FLERR, e.what());
      }
    } else {
...
void PairDeepMDNew::settings(int narg, char **arg) {
  if (narg <= 0) {
...
void PairDeepMDNew::read_restart(FILE *) { is_restart = true; }

void PairDeepMDNew::write_restart(FILE *) {
  // pass
}
...
void PairDeepMDNew::coeff(int narg, char **arg) {
  if (!allocated) {
    allocate();
  }
...
double PairDeepMDNew::init_one(int i, int j) {
  if (i > numb_types || j > numb_types) {
    char warning_msg[1024];
    sprintf(warning_msg,
...
int PairDeepMDNew::pack_reverse_comm(int n, int first, double *buf) {
  int i, m, last;
...
void PairDeepMDNew::unpack_reverse_comm(int n, int *list, double *buf) {
  int i, j, m;
...
void *PairDeepMDNew::extract(const char *str, int &dim) {
...
void PairDeepMDNew::extend(int &extend_inum,
